...
PinConfig::PinConfig(gpio_type *_port, uint16_t _pin)
    : port(_port), pin(_pin) {}

PinConfig::PinConfig(void) : port(nullptr), pin(0) {}

PinConfig::~PinConfig() {}

PinControl::PinControl(gpio_type *_port, uint16_t _pin, PinMode_t _mode,
                       gpio_pull_type _pull)
    : config(_port, _pin), state(false), mode(_mode), pull(_pull) {
  pinInit(config.port, static_cast<uint32_t>(config.pin), mode, pull);
}
...
bool PinControl::Get() {
  return gpio_output_data_bit_read(config.port,
                                   static_cast<uint32_t>(config.pin));
}

void PinControl::Toggle() { Set(!state); }

void PinControl::Reinit(gpio_type *gpio_x, uint32_t pins, PinMode_t mode,
                        gpio_pull_type pull) {
  this->config.port = gpio_x;
  this->config.pin = pins;
  this->mode = mode;
  this->pull = pull;
  pinInit(this->config.port, static_cast<uint32_t>(this->config.pin),
          this->mode, this->pull);
}