...
// ----------------------------------------------------------------------------
const uint8_t CAYENNE_LPP_BUFF_SIZE = 25;
uint8_t cayenne_lpp_buffer[CAYENNE_LPP_BUFF_SIZE] = {0};
...
CayenneLPP::CayenneLPP(void) : _maxsize(CAYENNE_LPP_BUFF_SIZE)
#ifndef ARDUINO
                                       ,
                                       _polyline(CAYENNE_LPP_BUFF_SIZE - 2)
#endif
{
  _buffer = cayenne_lpp_buffer;
  _cursor = 0;
}
...
template <typename T>
uint8_t CayenneLPP::addField(uint8_t type, uint8_t channel, T value)
{
